Feature: SearchBox Functionality check
  ...
  @smokeTest
  Scenario: AT&T searchBox functionality check with valid product
    Given I am on AT&T homePage
    And I click in searchBox
    And I input "SamSung Phones" in searchBox
    When I click on search box Button
    Then I must see "SamSung Phones" is properly appear
    But I must not see "Apple Phones" is appear
    And I do verify "Samsung Phones - Buy the Newest Galaxy Phones from AT&T" in product title
